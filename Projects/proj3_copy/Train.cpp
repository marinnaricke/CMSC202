...
void Train::addCar(TrainCar::cType car)
{
  ...
  if (car == 0)
    ...
    {
      //CoachClass TrainCar goes after Locomotive or after Business
      while (current != NULL)
	{
	  if (current->getType() == 0)
	    {
	      //if a BusinessClass car exists, place after
	      if (getNumBusiness() > 0)
		{
    ...
    {
      // Must be at least one or more sleeping cars
      if (getNumSleeping() >= 1)
	{
	  while (current->m_next != NULL)
	    {
	      // Places the Dining Car before the sleeping car
	      if (current->m_next->getType() == 5)
		{
		  tempCar = new TrainCar(car);
		  tempCar->m_next = current->m_next;
		  current->m_next = tempCar;
